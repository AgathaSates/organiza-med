...
import { DetalhesAtividadeMedicaModel } from '../../atividades-medicas.models';
...
export class ExcluirAtividadeMedica {
  ...
  protected readonly atividadeMedicaService = inject(AtividadesMedicasService);
  ...
  public excluir() {
    ...
    this.atividadeMedica$
      .pipe(
        take(1),
        switchMap((atividade) => this.atividadeMedicaService.excluir(atividade.dados.id)),
      )
